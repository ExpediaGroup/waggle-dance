...
| `primary-meta-stores.database-name-mapping`             | No       | BiDirectional Map of database names and mapped names. key=`<database name as known in the primary metastore>`, value=`<name that should be shown to a client>`. See the [Database Name Mapping](#database-name-mapping) section.|
