...
public final class AWSGlueClientFactory implements GlueClientFactory {
  ...
  public static final String AWS_GLUE_ENDPOINT = "factory.com.hotels.bdp.waggledance.com.hotels.bdp.waggledance.factory.glue.endpoint";

  public static final String AWS_REGION = "factory.region";
  public static final String AWS_CATALOG_CREDENTIALS_PROVIDER_FACTORY_CLASS = "factory.catalog.factory.provider.factory.class";
  ...
  private AWSCredentialsProvider getAWSCredentialsProvider(HiveConf conf) {
    Class<? extends AWSCredentialsProviderFactory> providerFactoryClass = conf
        .getClass("factory.catalog.factory.provider.factory.class", DefaultAWSCredentialsProviderFactory.class)
  ...
  public AWSGlue newClient() throws MetaException {
    try {
      ...
      String regionStr = getProperty("factory.region", conf);
      String glueEndpoint = getProperty("factory.com.hotels.bdp.waggledance.com.hotels.bdp.waggledance.factory.glue.endpoint", conf);

      if (StringUtils.isNotBlank(glueEndpoint)) {
        logger.info("Setting com.hotels.bdp.waggledance.com.hotels.bdp.waggledance.factory.glue service endpoint to " + glueEndpoint);
