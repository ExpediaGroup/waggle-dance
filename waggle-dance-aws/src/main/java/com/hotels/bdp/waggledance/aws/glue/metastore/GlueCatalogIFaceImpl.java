...
import com.hotels.bdp.waggledance.aws.glue.factory.AWSGlueClientFactory;
...
import com.hotels.bdp.waggledance.aws.glue.converter.util.BatchDeletePartitionsHelper;
import com.hotels.bdp.waggledance.aws.glue.converter.util.ExpressionHelper;
import com.hotels.bdp.waggledance.aws.glue.converter.util.LoggingHelper;
...
public class GlueCatalogIFaceImpl implements CloseableThriftHiveMetastoreIface {
  ...
  public GlueCatalogIFaceImpl(HiveConf conf) {
      this.conf = conf;
      open();
  }
  ...
  public boolean isOpen() {
    return glueClient != null && warehouse != null && glueMetastoreClientDelegate != null && currentMetaVars != null;
  }
  ...
  public void shutdown() {
    this.glueClient.shutdown();
    this.currentMetaVars = null;
    this.warehouse = null;
    this.glueMetastoreClientDelegate = null;
  }
    ...
    for (int i = 0; i < numOfPartitionsToDelete; i += 25) {
      ...
      batchDeletePartitionsFutures.add(BATCH_DELETE_PARTITIONS_THREAD_POOL.submit(new Callable() {
        public BatchDeletePartitionsHelper call() throws Exception {
          return new BatchDeletePartitionsHelper(GlueCatalogIFaceImpl.this.glueClient, dbName, tableName,
